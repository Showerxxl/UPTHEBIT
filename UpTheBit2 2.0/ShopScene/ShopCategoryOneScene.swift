...
class ShopCategoryOneScene: SKScene {
    ...
    override func didMove(to view: SKView) {
        ...
        let boxItems = shopEntity.boxListWeapon
        ...
        for index in 0..<boxItems.count {
            ...
            boxButton.name = "boxButton_1_\(index)"
            ...
            let swordImageName = "sword\(index + 1)"
            let swordSprite = SKSpriteNode(imageNamed: swordImageName)
     
            swordSprite.size = CGSize(width: boxButton.size.width * 0.8, height: boxButton.size.height * 0.8)
            swordSprite.position = CGPoint.zero
            swordSprite.zPosition = 2
            boxButton.addChild(swordSprite)
            addChild(boxButton)
            
            let damageY = boxY - boxButtonHeight * 0.8
            for index in 0..<boxItems.count {
                let xPosition = startX + CGFloat(index) * (boxButtonWidth + horizontalSpacing)
                let damageSprite = SKSpriteNode(imageNamed: "damage\(index + 1)")
                damageSprite.size = CGSize(width: boxButtonWidth, height: boxButtonHeight * 0.3)
                damageSprite.position = CGPoint(x: xPosition + boxButtonWidth / 2, y: damageY)
                damageSprite.zPosition = 1
                addChild(damageSprite)
            }
            
            let cY = damageY - boxButtonHeight * 0.3
            for index in 0..<boxItems.count {
               let xPosition = startX + CGFloat(index) * (boxButtonWidth + horizontalSpacing)
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for node in tappedNodes {
            if let nodeName = node.name, nodeName.hasPrefix("boxButton_1_") {
